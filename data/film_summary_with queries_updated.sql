...
SELECT title, category_name, SUM(total_revenue) AS Total_Revenue
FROM film_summary
...
FROM (
    SELECT category_name, title, SUM(total_rentals) AS total_rentals
    FROM film_summary
...
ORDER BY fs1.category_name ASC;
...
FROM (
    -- Total revenue per film within each category
    SELECT category_name, title, SUM(total_revenue) AS total_revenue
    FROM film_summary
    GROUP BY category_name, title
) fs1
JOIN (
    -- Maximum revenue per category
...
SELECT * 
INTO OUTFILE '/Users/ashwaursani/Documents/Data Warehouse/Project/film_summary.csv'
